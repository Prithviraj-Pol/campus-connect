...
const StudentPortal = () => {
  const { user, events, registerForEvent, isRegistered } = useApp();
                ...
                <Card key={event.id} className="shadow-md hover:shadow-lg transition-all border-0 animate-fade-in" style={{ animationDelay: `${i * 0.08}s` }}>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {registered ? (
                      ...
                      <Button className="w-full bg-accent text-accent-foreground hover:bg-accent/90 font-semibold shadow hover:shadow-md transition-all hover:-translate-y-0.5" onClick={() => registerForEvent(event.id)}>
                        Register Now
                      </Button>
                    )}
                  </CardContent>
